<template>
  <base-card>
    <h2>Vue User Inputs</h2>
    <form v-on:submit.prevent="submitForm">
      <div class="form-control">
        <label for="user-name">Your Name</label>
        <input
          v-model.trim="userName"
          id="user-name"
          name="user-name"
          type="text"
        />
        <p v-if="!userNameValidation">Please enter valid username</p>
      </div>
      <div class="form-control">
        <label for="user-age">Your Age (Years)</label>
        <input id="user-age" name="user-age" type="number" />
      </div>
      <div class="form-control">
        <label for="referrer">How did you learn about us?</label>
        <select id="referrer" name="referrer" v-model="selectInput">
          <option value="google">Google</option>
          <option value="newspaper">Newspaper</option>
          <option value="friend">Friend</option>
        </select>
      </div>
      <div class="form-control">
        <h3>What are you interested in?</h3>
        <div class="form-control-radio">
          <div>
            <input
              v-model="checklist"
              value="news"
              id="news"
              name="news"
              type="checkbox"
            />
            <label for="news">News</label>
          </div>
          <div>
            <input
              v-model="checklist"
              value="games"
              id="games"
              name="games"
              type="checkbox"
            />
            <label for="games">Games</label>
          </div>
          <div>
            <input
              v-model="checklist"
              value="podcasts"
              id="podcasts"
              name="podcasts"
              type="checkbox"
            />
            <label for="podcasts">Podcasts</label>
          </div>
        </div>
      </div>

      <div class="form-control">
        <h3>Select your payment type</h3>
        <div class="form-control-radio">
          <div>
            <input
              v-model="radio"
              name="payment"
              id="credit-card"
              type="radio"
              value="credit-card"
            />
            <label for="credit-card">Credit/Debit Card</label>
          </div>
          <div>
            <input
              v-model="radio"
              name="payment"
              id="online"
              value="online"
              type="radio"
            />
            <label for="online">Online Banking</label>
          </div>
          <div>
            <input
              v-model="radio"
              name="payment"
              value="cod"
              id="cod"
              type="radio"
            />
            <label for="cod">Pay on Delivery</label>
          </div>
        </div>
      </div>

      <div class="form-control">
        <div
          class="form-control-radio"
          style="display: flex; align-items: center"
        >
          <input
            v-model="terms"
            value="terms"
            id="terms"
            name="terms"
            type="checkbox"
          />
          <label
            style="margin-left: 10px; user-select: none; margin-bottom: 0"
            for="terms"
            >Please accept terms and conditions.</label
          >
        </div>
      </div>

      <the-rating-form v-model="rating" />

      <div class="form-control">
        <base-button
          style="max-width: 400px; margin: auto"
          mode="secondary"
          type="submit"
          >Save Details</base-button
        >
      </div>
    </form>
  </base-card>
</template>

<script>
import TheRatingForm from "./TheRatingForm";

export default {
  components: {
    "the-rating-form": TheRatingForm,
  },
  data() {
    return {
      checklist: [],
      radio: null,
      terms: false,
      selectInput: "newspaper",
      rating: "",
      userName: "",
      isUserNameValid: false,
    };
  },
  methods: {
    submitForm() {
      console.log(this.rating);
    },
  },
  computed: {
    userNameValidation() {
      if(this.isUserNameValid) {
        return true;
      } else {
        return this.userName === '';
      }
    },
  },
  watch: {
    userName(newVal) {
      this.isUserNameValid = newVal.length > 4;
    },
  },
};
</script>

<style>
.form-control {
  padding: 1rem;
  display: flex;
  flex-direction: column;
  color: #333;
}

input,
select {
  padding: 0.5rem 0.75rem;
  border: solid 1px #ccc;
  border-radius: 5px;
  background: #f0f0f0;
  box-shadow: 1px 1px 5px #aaa inset, -5px -5px 10px 3px #fff inset;
  outline-color: rgb(170, 170, 170);
  font-size: 16px;
}

.form-control-radio input {
  outline-color: #375bdd;
  box-shadow: none;
  background: #f0f0f0;
  border: solid 1px #ccc;
  height: 18px;
  width: 18px;
  cursor: pointer;
}

.form-control-radio div {
  display: flex;
  align-items: center;
  padding-bottom: 10px;
}

.form-control-radio div label {
  font-weight: 400;
  margin-left: 10px;
  margin-bottom: 0;
  user-select: none;
  cursor: pointer;
}

.form-control label {
  margin-bottom: 12px;
  font-size: 16px;
  font-weight: bold;
}

h3 {
  margin-top: 0;
  color: #333;
}
</style>